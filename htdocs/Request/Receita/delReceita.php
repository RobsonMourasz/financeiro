...
if(isset($_GET['idCR']) && !empty($_GET['idCR'])){
    if (isset($_GET['todos']) && !empty($_GET['todos'])) {
        ...
        if ($Parcelado == "S"){
            try {
                $selectControle = $conexao->query("SELECT * FROM cr_lancamentos WHERE idCR = $id ");
            ...
            try {
                $conexao->query("DELETE FROM cr_lancamentos WHERE Controle LIKE '$controle' AND DataVencimento >= '$data'");
            ...
            try {
                $conexao->query("DELETE FROM cr_lancamentos WHERE idCR = $id");
                $retorno = array("Retorno"=> "OK", "Motivo"=> "Registro excluido!");
            } catch (\Throwable $th) {
                $retorno = array("Retorno" => "ERRO", "Motivo" => "SQL(DELETE): ". $th->getMessage());
            }
